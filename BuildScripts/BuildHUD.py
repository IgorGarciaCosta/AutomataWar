...
simulation_screen.set_brush_color(BACKGROUND)
...
simulation_arena_host = make(unreal.Overlay, "SimulationArenaHost")
simulation_arena_size = make(unreal.SizeBox, "SimulationArenaSquare")
simulation_arena_size.set_width_override(560.0)
simulation_arena_size.set_height_override(560.0)
...
simulation_arena_viewport.set_brush_color(BACKGROUND)
simulation_arena_feed = make(unreal.Image, "SimulationArenaFeed", True)
simulation_arena_feed.set_color_and_opacity(WHITE)
simulation_arena_viewport.add_child(simulation_arena_feed)
...
simulation_arena_size.add_child(simulation_arena_frame)
add(simulation_arena_host, simulation_arena_size, h=H_CENTER, v=V_CENTER)
add(simulation_workspace, simulation_arena_host, fill=True,
    padding=margin(10, 0, 10, 0), weight=1.0)
...
replay_backdrop.set_brush_color(BACKGROUND)
...
add(transport_row, spacer("ReplayTransportSpace"), fill=True, weight=1.0)
add(transport_row, button("ReplayBackToMenuButton", "MENU", PANEL_ALT,
                          "Return to main menu", True), v=V_CENTER)
add(transport_row, button("NextRoundButton", "NEXT ROUND", GREEN,
...
replay_arena_host = make(unreal.Overlay, "ReplayArenaHost")
replay_arena_size = make(unreal.SizeBox, "ReplayArenaSquare")
replay_arena_size.set_width_override(560.0)
replay_arena_size.set_height_override(560.0)
...
replay_arena_viewport.set_brush_color(BACKGROUND)
replay_arena_feed = make(unreal.Image, "ReplayArenaFeed", True)
replay_arena_feed.set_color_and_opacity(WHITE)
replay_arena_viewport.add_child(replay_arena_feed)
...
replay_arena_size.add_child(replay_arena_frame)
add(replay_arena_host, replay_arena_size, h=H_CENTER, v=V_CENTER)
add(source_row, replay_arena_host, fill=True, weight=1.0)
...
background.modify()
background.set_editor_property("brush_color", BACKGROUND)
