...
FAWSyntaxHighlighter::ETokenType FAWSyntaxHighlighter::ClassifyToken(const FString &Token, bool bIsFirstToken, bool bHasColon)
{
    ...
    if (NumCheck.Len() > 0)
    {
        ...
        if (NumCheck[0] == TEXT('-'))
            Start = 1;
        if (Start >= NumCheck.Len())
            bIsNumber = false;
        for (int32 i = Start; i < NumCheck.Len() && bIsNumber; ++i)
        {
            if (!FChar::IsDigit(NumCheck[i]))
                bIsNumber = false;
        }
        if (bIsNumber)
    ...
    if (!bIsFirstToken)
    {
        ...
        if (bAllAlphaUnder && !Upper.IsEmpty() && FChar::IsAlpha(Upper[0]) && !InstructionSet.Contains(Upper) && !RegisterSet.Contains(Upper))
        {
            return ETokenType::Label;
        }
    }
