...
const TCHAR* EventName(Automata::EventType Type)
{
	switch (Type)
	{
	case Automata::EventType::Move: return TEXT("moved");
	case Automata::EventType::MoveBlockedWall: return TEXT("bumped wall");
	case Automata::EventType::MoveBlockedCover: return TEXT("blocked by cover");
	case Automata::EventType::MoveBlockedRobot: return TEXT("blocked by robot");
	case Automata::EventType::Turn: return TEXT("turned");
	case Automata::EventType::Scan: return TEXT("scanned");
	case Automata::EventType::Fire: return TEXT("fired");
	case Automata::EventType::ShieldActivate: return TEXT("shield activated");
	case Automata::EventType::ShieldAbsorb: return TEXT("shield absorbed hit");
	case Automata::EventType::Hit: return TEXT("hit");
	case Automata::EventType::ProjectileBlocked: return TEXT("projectile blocked");
	case Automata::EventType::Wait: return TEXT("waited");
	case Automata::EventType::Halt: return TEXT("halted");
	case Automata::EventType::EnergyDepleted: return TEXT("out of energy");
...
{
	TSharedRef<SWidgetSwitcher> Switcher = SAssignNew(ScreenSwitcher, SWidgetSwitcher)
		+ SWidgetSwitcher::Slot()[BuildMainMenu()]
		+ SWidgetSwitcher::Slot()[BuildProgrammingScreen()]
		+ SWidgetSwitcher::Slot()[BuildSimulationScreen()]
		+ SWidgetSwitcher::Slot()[BuildReplayAutopsyScreen()]
		+ SWidgetSwitcher::Slot()[BuildReplayBrowser()]
		+ SWidgetSwitcher::Slot()[BuildLanguageReference()];
...
TSharedRef<SWidget> UAWHUDWidget::BuildMainMenu()
{
	return SNew(SVerticalBox)
		...
		[
			SNew(SButton).Text(FText::FromString(TEXT("Local Match")))
			.OnClicked_Lambda([this]() { OnLocalMatch(); return FReply::Handled(); })
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 5)
		[
			SNew(SButton).Text(FText::FromString(TEXT("Host LAN")))
			.OnClicked_Lambda([this]() { OnHostLAN(); return FReply::Handled(); })
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 5)
		[
			SNew(SButton).Text(FText::FromString(TEXT("Find LAN")))
		...
		[
			...
			[
				SAssignNew(JoinIPField, SEditableTextBox)
				.HintText(FText::FromString(TEXT("IP Address...")))
			]
			+ SHorizontalBox::Slot().AutoWidth().Padding(4, 0, 0, 0)
			[
				SNew(SButton).Text(FText::FromString(TEXT("Join IP")))
				.OnClicked_Lambda([this]() { OnJoinIP(); return FReply::Handled(); })
			]
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 5)
		[
			SNew(SButton).Text(FText::FromString(TEXT("Replay Browser")))
			.OnClicked_Lambda([this]() { OnReplayBrowserNav(); return FReply::Handled(); })
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 5)
		[
			SNew(SButton).Text(FText::FromString(TEXT("Language Reference")))
			.OnClicked_Lambda([this]() { OnLanguageRef(); return FReply::Handled(); })
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 5)
		[
			SNew(SButton).Text(FText::FromString(TEXT("Quit")))
...
TSharedRef<SWidget> UAWHUDWidget::BuildProgrammingScreen()
{
	return SNew(SHorizontalBox)
		...
		[
			...
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("PLAYER 1")))
				.ColorAndOpacity(FSlateColor(AWUIColors::AccentCyan))
			]
			...
			[
				...
				[ SNew(SButton).Text(FText::FromString(TEXT("Submit"))).OnClicked_Lambda([this]() { OnSubmitSlot(0); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Aggressor"))).OnClicked_Lambda([this]() { OnLoadExample(0, TEXT("Aggressor")); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Camper"))).OnClicked_Lambda([this]() { OnLoadExample(0, TEXT("Camper")); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Kiter"))).OnClicked_Lambda([this]() { OnLoadExample(0, TEXT("Kiter")); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Train"))).OnClicked_Lambda([this]() { OnTrainingBot(0); return FReply::Handled(); }) ]
			]
		]
		...
		[
			...
			+ SVerticalBox::Slot().AutoHeight()
			[
				SNew(STextBlock).Text(FText::FromString(TEXT("PLAYER 2")))
			...
			[
				SNew(SHorizontalBox)
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Submit"))).OnClicked_Lambda([this]() { OnSubmitSlot(1); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Aggressor"))).OnClicked_Lambda([this]() { OnLoadExample(1, TEXT("Aggressor")); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Camper"))).OnClicked_Lambda([this]() { OnLoadExample(1, TEXT("Camper")); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Kiter"))).OnClicked_Lambda([this]() { OnLoadExample(1, TEXT("Kiter")); return FReply::Handled(); }) ]
				+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
				[ SNew(SButton).Text(FText::FromString(TEXT("Train"))).OnClicked_Lambda([this]() { OnTrainingBot(1); return FReply::Handled(); }) ]
			]
		];
}
...
TSharedRef<SWidget> UAWHUDWidget::BuildSimulationScreen()
{
	return SNew(SBox).HAlign(HAlign_Center).VAlign(VAlign_Center)
		[
			SNew(STextBlock).Text(FText::FromString(TEXT("SIMULATING...")))
...
TSharedRef<SWidget> UAWHUDWidget::BuildReplayAutopsyScreen()
{
	return SNew(SVerticalBox)
		...
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("|<"))).OnClicked_Lambda([this]() { OnReplayScrub(0); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("<"))).OnClicked_Lambda([this]() { OnReplayStepBack(); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("||"))).OnClicked_Lambda([this]() { OnReplayPause(); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT(">"))).OnClicked_Lambda([this]() { OnReplayPlay(); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT(">|"))).OnClicked_Lambda([this]() { OnReplayStepTick(); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("P1 >|"))).OnClicked_Lambda([this]() { OnReplayStepInstruction(0); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("P2 >|"))).OnClicked_Lambda([this]() { OnReplayStepInstruction(1); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(8, 0)[ SNew(SButton).Text(FText::FromString(TEXT(".25x"))).OnClicked_Lambda([this]() { OnReplaySetSpeed(0.25f); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("1x"))).OnClicked_Lambda([this]() { OnReplaySetSpeed(1.f); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("2x"))).OnClicked_Lambda([this]() { OnReplaySetSpeed(2.f); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("4x"))).OnClicked_Lambda([this]() { OnReplaySetSpeed(4.f); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().FillWidth(1.f).Padding(8, 0)
			[
				SAssignNew(ReplayScrubSlider, SSlider)
		...
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().AutoWidth()[ SAssignNew(ReplayTickText, STextBlock).Text(FText::FromString(TEXT("Tick: 0/0"))).ColorAndOpacity(FSlateColor(AWUIColors::TextPrimary)) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(16, 0, 0, 0)[ SAssignNew(ReplaySpeedText, STextBlock).Text(FText::FromString(TEXT("Speed: 1x"))).ColorAndOpacity(FSlateColor(AWUIColors::TextSecondary)) ]
			+ SHorizontalBox::Slot().FillWidth(1.f).Padding(16, 0, 0, 0)[ SAssignNew(ReplayOutcomeText, STextBlock).Text(FText::GetEmpty()).ColorAndOpacity(FSlateColor(AWUIColors::AccentCyan)) ]
		]
		...
		[
			...
			[
				SNew(SVerticalBox)
				+ SVerticalBox::Slot().AutoHeight()[ SNew(STextBlock).Text(FText::FromString(TEXT("P1 Source"))).ColorAndOpacity(FSlateColor(AWUIColors::AccentCyan)) ]
				+ SVerticalBox::Slot().FillHeight(1.f)
				[
					SNew(SScrollBox)
					+ SScrollBox::Slot()[ SAssignNew(ReplaySourceAText, STextBlock).Font(FCoreStyle::GetDefaultFontStyle("Mono", 10)).ColorAndOpacity(FSlateColor(AWUIColors::TextPrimary)) ]
				]
				+ SVerticalBox::Slot().AutoHeight().Padding(0, 4, 0, 0)
				[ SAssignNew(ReplayRegistersP1, STextBlock).Font(FCoreStyle::GetDefaultFontStyle("Mono", 10)).ColorAndOpacity(FSlateColor(AWUIColors::AccentCyan)) ]
			]
			// Right panel: P2 source + registers
			+ SHorizontalBox::Slot().FillWidth(1.f).Padding(4, 0, 0, 0)
			[
				SNew(SVerticalBox)
				+ SVerticalBox::Slot().AutoHeight()[ SNew(STextBlock).Text(FText::FromString(TEXT("P2 Source"))).ColorAndOpacity(FSlateColor(AWUIColors::AccentCoral)) ]
				+ SVerticalBox::Slot().FillHeight(1.f)
				[
					SNew(SScrollBox)
					+ SScrollBox::Slot()[ SAssignNew(ReplaySourceBText, STextBlock).Font(FCoreStyle::GetDefaultFontStyle("Mono", 10)).ColorAndOpacity(FSlateColor(AWUIColors::TextPrimary)) ]
				]
				+ SVerticalBox::Slot().AutoHeight().Padding(0, 4, 0, 0)
				[ SAssignNew(ReplayRegistersP2, STextBlock).Font(FCoreStyle::GetDefaultFontStyle("Mono", 10)).ColorAndOpacity(FSlateColor(AWUIColors::AccentCoral)) ]
			]
		]
		+ SVerticalBox::Slot().AutoHeight().MaxHeight(120.f).Padding(8, 2)
		[
			SNew(SScrollBox)
			+ SScrollBox::Slot()[ SAssignNew(ReplayEventLog, STextBlock).Font(FCoreStyle::GetDefaultFontStyle("Mono", 9)).ColorAndOpacity(FSlateColor(AWUIColors::TextSecondary)) ]
		]
...
TSharedRef<SWidget> UAWHUDWidget::BuildReplayBrowser()
{
	return SNew(SVerticalBox)
		...
		[
			SNew(STextBlock).Text(FText::FromString(TEXT("REPLAY BROWSER")))
		...
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
			[ SNew(SButton).Text(FText::FromString(TEXT("< Back"))).OnClicked_Lambda([this]() { ShowScreen(EAWScreen::MainMenu); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
			[ SNew(SButton).Text(FText::FromString(TEXT("Refresh"))).OnClicked_Lambda([this]() { RefreshReplayList(); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)
			[ SNew(SButton).Text(FText::FromString(TEXT("Save Current"))).OnClicked_Lambda([this]() { OnReplaySave(); return FReply::Handled(); }) ]
		]
		+ SVerticalBox::Slot().FillHeight(1.f).Padding(10, 4)
		[
			SNew(SScrollBox)
			+ SScrollBox::Slot()[ SAssignNew(ReplayListBox, SVerticalBox) ]
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 4)
		[
			SNew(SHorizontalBox)
		...
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().FillWidth(1.f)
			[ SAssignNew(ImportField, SEditableTextBox).HintText(FText::FromString(TEXT("Paste base64 replay here..."))) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(4, 0, 0, 0)
			[ SNew(SButton).Text(FText::FromString(TEXT("Import"))).OnClicked_Lambda([this]() { OnReplayImport(); return FReply::Handled(); }) ]
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 2)
		[
...
TSharedRef<SWidget> UAWHUDWidget::BuildLanguageReference()
{
	TSharedRef<SVerticalBox> Content = SNew(SVerticalBox)
		+ SVerticalBox::Slot().AutoHeight().Padding(10)
		[
			SNew(STextBlock).Text(FText::FromString(TEXT("LANGUAGE REFERENCE")))
			.Font(FCoreStyle::GetDefaultFontStyle("Bold", 18))
			.ColorAndOpacity(FSlateColor(AWUIColors::TextPrimary))
		]
		+ SVerticalBox::Slot().AutoHeight().Padding(10, 2)
		[
			SNew(STextBlock).Text(FText::FromString(TEXT("Instructions (8 total):")))
			.ColorAndOpacity(FSlateColor(AWUIColors::TextSecondary))
		];
	...
	[
		SNew(STextBlock).Text(FText::FromString(FString::Printf(TEXT("Registers (%d total):"), Automata::TotalRegisterCount)))
		.ColorAndOpacity(FSlateColor(AWUIColors::TextSecondary))
	];
...
{
	if (UAWGameSubsystem* Sub = GetSubsystem()) { Sub->StartLocalMatch(); }
	ShowScreen(EAWScreen::Programming);
}

void UAWHUDWidget::OnHostLAN()
{
	if (UAWGameSubsystem* Sub = GetSubsystem()) { Sub->HostSession(TEXT("AutomataWar")); }
...
{
	if (!SessionListBox.IsValid()) return;
	...
	if (Sessions.IsEmpty()) SetStatus(TEXT("No LAN sessions found."), true);
}

void UAWHUDWidget::OnJoinIP()
{
	if (!JoinIPField.IsValid()) return;
	FString IP = JoinIPField->GetText().ToString().TrimStartAndEnd();
	if (IP.IsEmpty()) { SetStatus(TEXT("Enter an IP address."), true); return; }
	if (UAWGameSubsystem* Sub = GetSubsystem()) { Sub->JoinByIP(IP); }
...
{
	if (UAWGameSubsystem* Sub = GetSubsystem()) { Sub->NextRound(); }
}
...
void UAWHUDWidget::InitializeReplayFromGameState()
{
	...
	if (AAWArenaRenderer* Renderer = FindOrSpawnRenderer())
	{
		...
		for (auto c : ReplayController->GetGrid()) Grid.Add(c);
	...
	if (ReplayOutcomeText.IsValid())
	{
		...
		switch (R.outcome)
		{
		case Automata::MatchOutcome::Robot0Wins: Outcome = TEXT("P1 WINS"); break;
		case Automata::MatchOutcome::Robot1Wins: Outcome = TEXT("P2 WINS"); break;
		default: Outcome = TEXT("DRAW"); break;
		}
...
{
	if (!ReplayController.IsValid() || !ReplayController->IsValid()) return;
	...
	if (ReplayRegistersP1.IsValid()) ReplayRegistersP1->SetText(FText::FromString(FormatRegs(0)));
	if (ReplayRegistersP2.IsValid()) ReplayRegistersP2->SetText(FText::FromString(FormatRegs(1)));
...
void UAWHUDWidget::UpdateArenaFromReplay()
{
	...
	for (const auto& E : Events) UEEvents.Add(E);
...
{
	if (!ReplayListBox.IsValid()) return;
	...
	for (const FAWReplayInfo& Info : Replays)
	{
		...
		[
			SNew(SHorizontalBox)
			+ SHorizontalBox::Slot().FillWidth(1.f)[ SNew(STextBlock).Text(FText::FromString(Label)).ColorAndOpacity(FSlateColor(AWUIColors::TextPrimary)) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(4, 0)[ SNew(SButton).Text(FText::FromString(TEXT("Load"))).OnClicked_Lambda([this, Filename]() { OnReplayLoad(Filename); return FReply::Handled(); }) ]
			+ SHorizontalBox::Slot().AutoWidth().Padding(2, 0)[ SNew(SButton).Text(FText::FromString(TEXT("Export"))).OnClicked_Lambda([this, Filename]() { OnReplayExport(Filename); return FReply::Handled(); }) ]
		];
	}
...
void UAWHUDWidget::OnReplayLoad(const FString& Filename)
{
	...
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(Error));
		return;
	}
	...
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(TEXT("Failed to resimulate replay.")));
		return;
	}
	...
	if (ReplaySourceAText.IsValid())
	{
		TArray<FString> Lines; Src0.ParseIntoArrayLines(Lines);
		FString N; for (int32 i = 0; i < Lines.Num(); ++i) N += FString::Printf(TEXT("%3d| %s\n"), i+1, *Lines[i]);
		ReplaySourceAText->SetText(FText::FromString(N));
	}
	if (ReplaySourceBText.IsValid())
	{
		TArray<FString> Lines; Src1.ParseIntoArrayLines(Lines);
		FString N; for (int32 i = 0; i < Lines.Num(); ++i) N += FString::Printf(TEXT("%3d| %s\n"), i+1, *Lines[i]);
...
void UAWHUDWidget::OnReplaySave()
{
	...
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(FString::Printf(TEXT("Saved: %s"), *Filename)));
		RefreshReplayList();
	}
	else
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(TEXT("Save failed (not in ReplayAutopsy phase?).")));
	}
}
...
void UAWHUDWidget::OnReplayExport(const FString& Filename)
{
	...
	{
		if (ExportField.IsValid()) ExportField->SetText(FText::FromString(Base64));
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(TEXT("Exported to field above.")));
	}
	else
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(TEXT("Export failed.")));
	}
}

void UAWHUDWidget::OnReplayImport()
{
	if (!ImportField.IsValid()) return;
	FString Base64 = ImportField->GetText().ToString().TrimStartAndEnd();
	if (Base64.IsEmpty())
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(TEXT("Paste base64 data first.")));
		return;
	}
	...
	{
		if (ReplayBrowserStatus.IsValid()) ReplayBrowserStatus->SetText(FText::FromString(FString::Printf(TEXT("Imported: %s"), *Filename)));
		RefreshReplayList();
	}
	else
	{
