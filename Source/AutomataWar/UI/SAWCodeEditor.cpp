...
void SAWCodeEditor::Construct(const FArguments& InArgs)
{
	...
	FSlateFontInfo MonoFont = FCoreStyle::GetDefaultFontStyle("Mono", 12);
...
void SAWCodeEditor::RebuildGutter()
{
	...
	FSlateFontInfo GutterFont = FCoreStyle::GetDefaultFontStyle("Mono", 11);
...
{
	if (!DiagnosticsBox.IsValid()) return;
	...
	if (LastCompileResult.diagnostics.empty())
	{
		...
		if (OnCompileSuccess.IsBound()) OnCompileSuccess.Execute();
		return;
	}
