...
namespace Automata
{
    ...
    uint64_t Simulation::ComputeHash() const
    {
        ...
        for (int32_t i = 0; i < 2; ++i)
        {
            ...
            mix(robots_[i].hp);
            mix(robots_[i].energy);
            mix(robots_[i].shielded ? 1 : 0);
            mix(robots_[i].vm.pc);
            mix(robots_[i].vm.busyLeft);
    ...
    void Simulation::ResolveIntent(int32_t robotIdx, const Intent &intent, int32_t tick)
    {
        ...
        case IntentType::Scan:
        {
            ...
            if (dist <= ScanRange && dist > 0)
            {
                ...
                {
                case Dir::North:
                    inCone = (dy < 0 && std::abs(dx) <= std::abs(dy));
                    break;
                case Dir::South:
                    inCone = (dy > 0 && std::abs(dx) <= std::abs(dy));
                    break;
                case Dir::East:
                    inCone = (dx > 0 && std::abs(dy) <= std::abs(dx));
                    break;
                case Dir::West:
                    inCone = (dx < 0 && std::abs(dy) <= std::abs(dx));
                    break;
            ...
            if (detected)
            {
                ...
                r.vm.regs[static_cast<int>(Reg::R_ENEMY_DIR)] =
                    cross < 0 ? static_cast<int32_t>(RelativeEnemyDir::Left) : cross > 0 ? static_cast<int32_t>(RelativeEnemyDir::Right)
