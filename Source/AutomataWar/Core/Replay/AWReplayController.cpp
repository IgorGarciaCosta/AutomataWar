...
{
	if (!bValid_ || currentTick_ >= GetTotalTicks() - 1) return false;
	++currentTick_;
	return true;
}
...
{
	if (!bValid_ || currentTick_ <= 0) return false;
	--currentTick_;
	return true;
}
...
{
	if (!bValid_ || RobotIdx < 0 || RobotIdx > 1) return false;
	if (currentTick_ >= GetTotalTicks() - 1) return false;
...
std::vector<SimEvent> FAWReplayController::GetEventsInRange(int32_t FromTick, int32_t ToTick) const
{
	...
	for (const auto& e : events_)
	{
		if (e.tick >= FromTick && e.tick <= ToTick) result.push_back(e);
	}
	return result;
}
