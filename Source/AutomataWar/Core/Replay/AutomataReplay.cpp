...
uint32_t ReadU32(const uint8_t* p) { uint32_t v = 0; for (int i = 0; i < 4; ++i) v |= static_cast<uint32_t>(p[i]) << (i*8); return v; }
uint64_t ReadU64(const uint8_t* p) { uint64_t v = 0; for (int i = 0; i < 8; ++i) v |= static_cast<uint64_t>(p[i]) << (i*8); return v; }
...
std::vector<uint8_t> EncodeReplay(const ReplayData& data)
{
    ...
    // Magic.
    out.push_back('A'); out.push_back('W'); out.push_back('R'); out.push_back('P');
...
ReplayDecodeResult DecodeReplay(const std::vector<uint8_t>& bytes)
{
    ...
    result.data.version = ReadU16(p); p += 2;
    ...
    result.data.rulesetHash = ReadU64(p); p += 8;
    ...
    result.data.seed = ReadU64(p); p += 8;

    // Source A.
    size_t remaining = bytes.size() - static_cast<size_t>(p - bytes.data());
    if (remaining < 2) { result.error = ReplayError::Truncated; return result; }
    uint16_t lenA = ReadU16(p); p += 2;
    remaining -= 2;
    if (remaining < lenA) { result.error = ReplayError::Truncated; return result; }
    result.data.sourceA.assign(reinterpret_cast<const char*>(p), lenA); p += lenA;
    remaining -= lenA;

    // Source B.
    if (remaining < 2) { result.error = ReplayError::Truncated; return result; }
    uint16_t lenB = ReadU16(p); p += 2;
    remaining -= 2;
    if (remaining < lenB) { result.error = ReplayError::Truncated; return result; }
    result.data.sourceB.assign(reinterpret_cast<const char*>(p), lenB); p += lenB;
    remaining -= lenB;

    // CRC.
    if (remaining < 4) { result.error = ReplayError::Truncated; return result; }
...
bool ReplayFromBase64(const std::string& b64, std::vector<uint8_t>& outBytes)
{
    ...
    while (i < len)
    {
        ...
        if (pad < 2) outBytes.push_back(static_cast<uint8_t>((triple >> 8) & 0xFF));
        if (pad < 1) outBytes.push_back(static_cast<uint8_t>(triple & 0xFF));
    }
    return true;
}
